...
struct program_stop
{
    ...
    operator stop_listener() { return stop_listener(latch_); }
    ...
    void
    stop(std::exception &e)
    {
        stop(127, e.what());
    }

    void
    stop(int retcode, std::string msg)
    {
        if (!stopped())
        {
            code_    = retcode;
...
void
report(std::exception const &e, std::string_view context, Idents &&...idents)
{
    std::cout << context;
    if constexpr (sizeof...(idents) != 0)
    {
        auto sep = "["sv;
        ((emit(std::cout, sep), emit(std::cout, idents), sep = ", "), ...);
        emit(std::cout, ']');
    }
    emit(std::cout, " : ");
...
detect_ssl(asio::ip::tcp::socket &sock, beast::flat_buffer &buf)
{
    ...
    cstate.slot().assign([&](asio::cancellation_type type) { sock.close(); });
...
chat(asio::ip::tcp::socket sock)
{
    ...
    try
    {
        ...
        auto which = co_await(detect_ssl(sock, buf) || timer.async_wait(asio::use_awaitable));
...
try
{
    ...
    while (!pstop.stopped())
    {
        ...
        auto which =
            co_await(acceptor.async_accept(sock, asioex::as_tuple(asio::use_awaitable)) || pstop(asio::use_awaitable));
        ...
        if (which.index() == 0)
        {
            ...
            asio::co_spawn(
                ...
                [](auto sock, auto &pstop) -> asio::awaitable< void > {
                    co_await(chat(std::move(sock)) || pstop(asio::use_awaitable));
                }(std::move(sock), pstop),
                asio::detached);
        }
    }
...
auto
monitor_sigint(program_stop &pstop) -> asio::awaitable< void >
{
